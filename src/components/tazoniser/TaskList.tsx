...
// Handles: add, check-off, remove, per-task notes, and sub-tasks.
...
export function TaskList({
  ...
  function toggleSubTasks(taskId: string) {
    setExpandedSubTaskId((prev) => (prev === taskId ? null : taskId));
  }
      ...
      {tasks.length === 0 ? (
        ...
        <ul className="space-y-2">
          {tasks.map((task) => {
            ...
            const subLabel = subTotal > 0 ? `Sub (${subDone}/${subTotal})` : "Sub";
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex-none flex items-center gap-1">
                    <button
                      ...
                      aria-label={`${expandedSubTaskId === task.id ? "Hide" : "Show"} sub-tasks for "${task.text}"`}
                    >
                      {subLabel}
                    </button>
                    <button
                ...
                {/* Sub-tasks panel */}
                {expandedSubTaskId === task.id && (
                  <div className="border-t border-calm-100 px-4 py-3 bg-slate-50 space-y-2">
                    {task.subTasks.length > 0 && (
                      <ul className="space-y-1.5">
                        {task.subTasks.map((st) => (
                          <li key={st.id} className="flex items-center gap-2">
                            <button
                              onClick={() => onToggleSubTask(task.id, st.id, !st.done)}
                              className={`flex-none w-4 h-4 rounded border-2 transition-colors flex items-center justify-center ${
                                st.done
                                  ? "bg-calm-500 border-calm-500"
                                  : "border-calm-300 hover:border-calm-500"
                              }`}
                              aria-label={st.done ? `Uncheck "${st.text}"` : `Check "${st.text}"`}
                            >
                              {st.done && (
                                <svg
                            ...
                            <span
                              className={`flex-1 text-xs leading-relaxed ${
                                st.done ? "line-through text-slate-400" : "text-slate-600"
                              }`}
                            >
                              {st.text}
                            </span>
                            <button
                              ...
                              className="flex-none p-0.5 text-slate-300 hover:text-red-400 transition-colors"
                              aria-label={`Remove sub-task "${st.text}"`}
                            >
                              <svg
                                className="w-3 h-3"
                                viewBox="0 0 16 16"
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-2.5 py-1.5 bg-white border border-calm-200 rounded-lg text-xs text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-calm-400 focus:border-transparent"
                      />
                      <button
                        onClick={() => handleAddSubTask(task.id)}
                        disabled={!subTaskDrafts[task.id]?.trim()}
                        className="px-2.5 py-1.5 bg-calm-600 hover:bg-calm-700 disabled:bg-calm-200 disabled:text-calm-400 text-white text-xs font-medium rounded-lg transition-colors"
                      >
                        Add
                      </button>
                    </div>
